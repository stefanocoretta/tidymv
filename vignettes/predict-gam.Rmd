...
While `plot_smooths()` offers a streamlined way of plotting predicted smooths from a GAM model, it is too constrained for other more complex cases.
...
The output still contains the excluded columns, and the predicted values of the oucome variable are the same independent of the value the excluded terms (the predicted values are repeated for each value of the excluded terms).
We can filter the predicted dataset by chosing any value or level of the excluded terms.
...
Of course, it is possible to plot the predicted values of random effects if the user wishes to.
In the following example, the random effect `rand` is not excluded when predicting, and it is used two wrap facets in the plot.
